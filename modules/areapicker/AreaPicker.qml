...
MouseArea {
  ...
  required property AreaPickerContext context
  required property var onRegion
  ...
  property list<var> clients: Hyprland.toplevels.values.filter(c => c.workspace?.id === Hyprland.focusedWorkspaceId).sort((a, b) => {
  ...
  onReleased: {
    ...
    onRegion(screen.x + Math.ceil(rsx), screen.y + Math.ceil(rsy), Math.floor(sw), Math.floor(sh), onClient);
    closeAnim.start();
  }
